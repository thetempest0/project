<template>
	<div class="unitVue">
		...
		<dialog-unit ref="dialogUnit" @click-refresh="setData"></dialog-unit>
	</div>
</template>
<script>
  ...
  export default {
    components: {
      'dialog-unit': require('./dialog-unit.vue')
    },
    data () {
      return {
        tableLoading: false,
    ...
    methods: {
      ...
      handleAdd () {
        this.$refs.dialogUnit.show(null)
      },
      handleEdit (index, row) {
        this.$refs.dialogUnit.show(JSON.parse(JSON.stringify(row)))
      },
