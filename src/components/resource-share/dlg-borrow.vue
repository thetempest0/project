  ...
  <el-dialog :title="title" v-model="visible" @close='close'>
    <section class="dialog-body">
      ...
      <el-row type="flex" justify="center" align="center">
        ...
        <el-col :span="8" class="vertical-center">
          <el-input-number class="small-input" :min="0" :max="maxNum" v-model="model.number"></el-input-number>
        </el-col>
      </el-row>
      ...
      <el-row type="flex" justify="center" align="center">
        ...
        <el-col :span="19" class="vertical-center">
          <el-input disabled class="middle-input" v-model="model.remark" placeholder="请输入备注说明"></el-input>
        </el-col>
      </el-row>
    ...
    <el-row>
      <section class="dialog-footer">
        ...
        <el-button @click="confirm">提交申请</el-button>
      </section>
    </el-row>
  </el-dialog>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      show (row) {
        this.srcData = row
        this.title = `借用申请 (${row.supplierName})`
        this.maxNum = parseInt(row.availableNum)
      ...
      confirm () {
        ...
        api.resourceShare.addDebitBillByOtherSupplier(resource.supplieId, resource.resourcesId,
          model.number, model.borrowDateTime, model.returnDateTime)
          .then(response => {
      ...
      checkError (model) {
        if (!model.borrowDateTime) {
          this.notify('error', '请填入借用日期')
          return false
        }
        if (!model.returnDateTime) {
          this.notify('error', '请填入归还日期')
          return false
        }
        if (model.number === 0) {
          this.notify('error', '借用数量不能为0')
          return false
        }
        ...
        if (dtReturn.getTime() < dtBorrow.getTime()) {
          this.notify('error', '归还日期不能早于借出日期')
          return false
        }
        return true
      },
