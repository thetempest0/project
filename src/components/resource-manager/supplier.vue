<template>
  <div class="table-border">
    <el-table :data="tableData" border stripe v-loading="tableLoading">
      ...
      <el-table-column label="备注说明" align="left" header-align="center" width="300">
      ...
      <el-table-column label="操作" align="center" header-align="center">
        <template scope="scope">
          <section v-if="scope.row.status === '1'">
            <el-button type="text" @click="btnEditClick('destruction', scope.$index, scope.row,scope.store)">删除
            </el-button>
          </section>
          <section v-if="scope.row.status === '2'">
            <el-button type="text" @click="btnEditClick('occupy', scope.row, scope.store)">占用</el-button>
            <span>|</span>
            <el-button type="text" @click="btnEditClick('change', scope.row,scope.store)">变更</el-button>
          </section>
          <section v-if="scope.row.status === '3'">
            <el-button type="text" @click="btnEditClick('occupy', scope.row,scope.store)">占用</el-button>
            <span>|</span>
            <el-button type="text" @click="btnEditClick('change', scope.row,scope.store)">变更
            </el-button>
          </section>
        </template>
      </el-table-column>
    ...
    <jk-dlg-destruction ref="destruction" @callback='callback'></jk-dlg-destruction>
    <jk-dlg-occupy ref="occupy" @callback='callback'></jk-dlg-occupy>
    <jk-dlg-change ref="change" @callback='callback'></jk-dlg-change>
  </div>
</template>
<script>
  ...
  export default {
    components: {
      ...
      'jk-dlg-destruction': require('./dlg-resource-destruction.vue'),
      'jk-dlg-occupy': require('./dlg-resource-occupy.vue'),
      'jk-dlg-change': require('./dlg-resource-change.vue')
    },
    props: {
      onlyPaddingAudit: {
        type: Boolean,
    ...
    methods: {
      ...
      setData (currentResource) {
        ...
        api.resourceManage.getSupplierList(param).then((response) => {
      ...
      btnEditClick (operate, row, store) {
        switch (operate) {
          case 'destruction':
            this.$refs.destruction.show(row, store)
            break
          case 'occupy':
            this.$refs.occupy.show(row, store)
            break
          case 'change':
            this.$refs.change.show(row, store)
            break
          default:
            break
        }
