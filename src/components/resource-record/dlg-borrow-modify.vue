  ...
  <el-dialog :title="title" v-model="visible" @close='close'>
    ...
    <el-row>
      <section class="dialog-footer">
        <el-button @click="deleteBill">撤销单据</el-button>
  ...
  export default {
    ...
    methods: {
      show (row) {
        ...
        this.model.number = row.borrowNum
      ...
      confirm () {
        ...
        api.resourceShare.updateBill(resource.id, model.number, model.borrowDateTime, model.returnDateTime)
          .then(response => {
      ...
      deleteBill () {
        api.resourceShare.deleteBill(this.srcData.id)
          .then(response => {
            const data = response.data
            if (data.meta.code === 100000) {
              this.$notify({
